...
public:
  ...
  void addSwapchain(const vdu::Swapchain &swapchain, uint32_t imageIndex,
                    const VkRect2D &srcRect =
                        VkRect2D({{std::numeric_limits<int32_t>::lowest(),
                    ...
                    const VkRect2D &dstRect =
                        VkRect2D({{std::numeric_limits<int32_t>::lowest(),
                                   std::numeric_limits<int32_t>::lowest()},
                                  {0, 0}}),
                    VkBool32 persistant = VK_FALSE);

  const std::vector<VkSemaphore> &getWaitSemaphores() const {
    return m_waitSemaphores;
  }
  const std::vector<VkSwapchainKHR> &getSwapchains() const {
    return m_swapchains;
  }
  const std::vector<uint32_t> &getImageIndices() const {
    return m_imageIndices;
  }
  const std::vector<VkResult> &getResults() const { return m_results; }
  const VkDisplayPresentInfoKHR &getDisplayPresentInfo() const {
...
public:
  ...
  VkResult submit(VkSubmitInfo *info, uint32_t count = 1,
                  VkFence fence = VK_NULL_HANDLE);
  VkResult submit(const QueueSubmission &qSubmit,
                  const vdu::Fence &fence = vdu::Fence());
  VkResult submit(const std::vector<QueueSubmission> &qSubmits,
                  const vdu::Fence &fence = vdu::Fence());
