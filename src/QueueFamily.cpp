...
vdu::QueueFamily::QueueFamily(PhysicalDevice *physicalDevice,
                              VkQueueFamilyProperties props,
                              uint32_t familyIndex)
    : m_physicalDevice(physicalDevice), m_familyProperties(props),
      m_familyIndex(familyIndex), m_supportsGraphics(false),
      m_supportsCompute(false), m_supportsTransfer(false),
...
VkResult vdu::QueueFamily::queryPresentCapability(VkSurfaceKHR surface) {
  ...
  auto result = vkGetPhysicalDeviceSurfaceSupportKHR(
      m_physicalDevice->getHandle(), m_familyIndex, surface, &surfaceSupport);
