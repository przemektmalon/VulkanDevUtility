...
void vdu::RenderPass::create(vdu::LogicalDevice *device) {
  ...
  subpass.pDepthStencilAttachment =
      m_depthAttachmentInfo ? &m_depthAttachmentInfo->getReference() : 0;
  ...
  VDU_VK_CHECK_RESULT(vkCreateRenderPass(m_logicalDevice->getHandle(),
...
vdu::RenderPass::AttachmentInfo *
vdu::RenderPass::addColourAttachment(vdu::Texture *texture, std::string name) {
  m_attachments.insert(std::make_pair(name, texture));
  auto insertion =
      m_attachmentInfos.insert(std::make_pair(name, new AttachmentInfo()));

  m_attachmentInfos[name]->setFormat(texture->getFormat());
  m_attachmentInfos[name]->setAttachmentIndex(m_attachments.size() - 1 +
                                              (m_depthAttachment ? 1 : 0));

  return insertion.first->second;
}

vdu::RenderPass::AttachmentInfo *
vdu::RenderPass::addColourAttachment(VkFormat format, std::string name) {
...
vdu::RenderPass::AttachmentInfo *
vdu::RenderPass::setDepthAttachment(vdu::Texture *texture) {
...
vdu::RenderPass::AttachmentInfo *
vdu::RenderPass::setDepthAttachment(VkFormat format) {
