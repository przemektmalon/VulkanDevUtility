...
void vdu::DescriptorPool::create(LogicalDevice *logicalDevice) {
  ...
  dpci.flags =
      m_freeable ? VK_DESCRIPTOR_POOL_CREATE_FREE_DESCRIPTOR_SET_BIT : 0;
  dpci.maxSets = m_maxSets;
  dpci.poolSizeCount = poolSizes.size();
  dpci.pPoolSizes = poolSizes.data();

  VDU_VK_CHECK_RESULT(vkCreateDescriptorPool(m_logicalDevice->getHandle(),
                                             &dpci, nullptr, &m_descriptorPool),
                      "creating descriptor pool");
}

void vdu::DescriptorPool::destroy() {
  vkDestroyDescriptorPool(m_logicalDevice->getHandle(), m_descriptorPool,
                          nullptr);
}
...
void vdu::DescriptorPool::addSetCount(uint32_t count) { m_maxSets += count; }

vdu::CommandPool::CommandPool()
    : m_commandPool(0), m_queueFamily(nullptr),
      m_flags(VkCommandPoolCreateFlagBits(0)), m_logicalDevice(nullptr) {}
...
void vdu::CommandPool::create(LogicalDevice *logicalDevice) {
  ...
  VDU_VK_CHECK_RESULT(vkCreateCommandPool(m_logicalDevice->getHandle(), &cpci,
...
vdu::QueryPool::QueryPool()
    : m_queryPool(0), m_logicalDevice(nullptr), m_pipelineStats(0),
      m_queryData(nullptr) {}
...
void vdu::QueryPool::create(LogicalDevice *logicalDevice) {
  ...
  VDU_VK_CHECK_RESULT(
      vkCreateQueryPool(m_logicalDevice->getHandle(), &ci, 0, &m_queryPool),
      "creating query pool");
...
void vdu::QueryPool::setQueryType(VkQueryType type) { m_type = type; }

void vdu::QueryPool::setPipelineStats(VkQueryPipelineStatisticFlags flags) {
  m_pipelineStats = flags;
}

void vdu::QueryPool::setQueryCount(uint32_t count) { m_count = count; }

uint64_t *vdu::QueryPool::query() { return query(0, m_count); }

uint64_t *vdu::QueryPool::query(uint32_t first, uint32_t count) {
  VDU_VK_CHECK_RESULT(
      vkGetQueryPoolResults(m_logicalDevice->getHandle(), m_queryPool, first,
                            count, sizeof(uint64_t) * count, m_queryData,
...
void vdu::QueryPool::cmdReset(const vdu::CommandBuffer &cmd, uint32_t first,
                              uint32_t count) {
...
void vdu::QueryPool::cmdReset(const VkCommandBuffer &cmd, uint32_t first,
                              uint32_t count) {
  vkCmdResetQueryPool(cmd, m_queryPool, 0, count);
}

void vdu::QueryPool::cmdTimestamp(const vdu::CommandBuffer &cmd,
                                  VkPipelineStageFlagBits flags,
                                  uint32_t index) {
  vkCmdWriteTimestamp(cmd.getHandle(), flags, m_queryPool, index);
}

void vdu::QueryPool::cmdTimestamp(const VkCommandBuffer &cmd,
                                  VkPipelineStageFlagBits flags,
                                  uint32_t index) {
