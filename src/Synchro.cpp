...
void vdu::Event::create(LogicalDevice *device) {
  ...
  VDU_VK_CHECK_RESULT(
      vkCreateEvent(m_logicalDevice->getHandle(), &eci, nullptr, &m_event),
      "creating event");
}
...
void vdu::Fence::create(LogicalDevice *device, bool initiallySignalled) {
  ...
  VDU_VK_CHECK_RESULT(
      vkCreateFence(m_logicalDevice->getHandle(), &fci, nullptr, &m_fence),
      "creating fence");
}
...
VkResult vdu::Fence::wait(uint64_t timeout) const {
  auto result =
      vkWaitForFences(m_logicalDevice->getHandle(), 1, &m_fence, true, timeout);
  if (result == VK_SUCCESS || result == VK_TIMEOUT)
    return result;
  else {
    m_logicalDevice->_internalReportVkError(
        result, "Encountered Vulkan error while waiting for fence");
  }
}
