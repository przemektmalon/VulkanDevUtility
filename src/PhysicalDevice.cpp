...
uint32_t vdu::PhysicalDevice::findMemoryTypeIndex(
...
VkFormat vdu::PhysicalDevice::findSupportedFormat(
...
vdu::PhysicalDevice::getSurfaceCapabilities() const {
  return m_surfaceCapabilities;
}

const std::vector<VkSurfaceFormatKHR> &
vdu::PhysicalDevice::getSurfaceFormats() const {
  return m_surfaceFormats;
}

const std::vector<VkPresentModeKHR> &
vdu::PhysicalDevice::getPresentModes() const {
...
VkResult vdu::PhysicalDevice::querySurfaceCapabilities(VkSurfaceKHR surface) {
  ...
  VK_CHECK_RESULT(vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
      m_physicalDevice, surface, &m_surfaceCapabilities));

  // Query surface formats
  {
    uint32_t formatCount;
    VK_CHECK_RESULT(vkGetPhysicalDeviceSurfaceFormatsKHR(
        m_physicalDevice, surface, &formatCount, nullptr));

    if (formatCount != 0) {
      m_surfaceFormats.resize(formatCount);
      VK_CHECK_RESULT(vkGetPhysicalDeviceSurfaceFormatsKHR(
          m_physicalDevice, surface, &formatCount, m_surfaceFormats.data()));
    } else
  ...
  // Query surface present modes
  {
    ...
    if (presentModeCount != 0) {
      ...
      VK_CHECK_RESULT(vkGetPhysicalDeviceSurfacePresentModesKHR(
          m_physicalDevice, surface, &presentModeCount, m_presentModes.data()));
    } else
      m_suitabilityScore -= 100000;
  }
