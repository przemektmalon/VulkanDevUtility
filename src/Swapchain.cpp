...
void vdu::Swapchain::create(LogicalDevice *device, VkSurfaceKHR surface) {
  ...
  if (surfaceCapabilities.currentExtent.width !=
    ...
    VkExtent2D actualExtent = {std::numeric_limits<uint32_t>::max(),
                               std::numeric_limits<uint32_t>::max()};
  ...
  // If we have several queue families then we will need to look into sharing
  // modes
  /*
  ...
  */
  ...
  VDU_VK_CHECK_RESULT(vkCreateSwapchainKHR(m_logicalDevice->getHandle(),
                                           &createInfo, nullptr, &m_swapchain),
                      "creating swapchain");

  VDU_VK_CHECK_RESULT(vkGetSwapchainImagesKHR(m_logicalDevice->getHandle(),
  ...
  for (uint32_t i = 0; i < m_imageCount; i++) {
    ...
    VDU_VK_CHECK_RESULT(vkCreateImageView(m_logicalDevice->getHandle(),
                                          &viewInfo, nullptr, &m_imageViews[i]),
  ...
  for (size_t i = 0; i < m_imageViews.size(); i++) {
    ...
    VDU_VK_CHECK_RESULT(vkCreateFramebuffer(m_logicalDevice->getHandle(),
...
void vdu::Swapchain::destroy() {
  for (int i = 0; i < m_images.size(); ++i) {
    // Gives error, maybe because these images are created from the swap chain
    // by KHR extension
...
const VkResult vdu::Swapchain::acquireNextImage(uint32_t &imageIndex,
                                                vdu::Semaphore signal,
                                                const Fence &fence,
                                                uint64_t timeout) {
  return vkAcquireNextImageKHR(m_logicalDevice->getHandle(), m_swapchain,
                               timeout, signal.getHandle(), fence.getHandle(),
                               &imageIndex);
}
